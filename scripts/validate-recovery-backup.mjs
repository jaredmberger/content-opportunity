...
if(backup.format!=='content-opportunity-kv-recovery')fail('Unexpected backup format.');
...
for(const entry of backup.data.entries){
  ...
  try{JSON.parse(entry.value);}catch{fail(`KV value is not valid JSON for ${entry.key}`);}
}
...
console.log('Content Opportunity recovery backup is valid.');
...
function fail(message){
  console.error(`Invalid Content Opportunity recovery backup: ${message}`);
  process.exit(1);
}
